...
class Solution:
    def makesquare(self, matchsticks: List[int]) -> bool:
        ...
        def r(position, s1, s2, s3, s4, memo):
            ...
            if r(position + 1, s1 + matchsticks[position],s2,s3,s4, memo): return True
            if r(position + 1, s1 ,s2+ matchsticks[position], s3,s4, memo): return True
            if r(position + 1, s1 ,s2, s3 + matchsticks[position],s4, memo): return True
            if r(position + 1, s1 ,s2, s3,s4 + matchsticks[position], memo): return True
