...
class Solution:
    def calc_max_path_sum(self, node):
        ...
        current_path = max(node.val, node.val + max(result_left[0], result_right[0])) if (node.left and node.right) else max(node.val + result_left[0] + result_right[0], node.val)
        
        current_sum = [node.val, node.val + result_left[0] + result_right[0], node.val + result_left[0], node.val + result_right[0]]
        if node.left:
